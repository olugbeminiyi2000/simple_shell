...
int main(int argc, char *argv[])
{
	...
	char *semicolon_commands[MAX_ARGUMENTS];
	/*char *args[MAX_ARGUMENTS];*/
	/*int num_args;*/
	int input_fd = STDIN_FILENO;
	ssize_t i;
	int num_semicolon_commands;
	size_t j;
	...
	while (1)
	{
		...
		for (i = 0; i < total_bytes_read; i++)
		{
			if (buffer[i] == '\n')
			{
				...
				/* char *semicolon_commands[MAX_ARGUMENTS];*/
				 num_semicolon_commands = split_by_separator(command, semicolon_commands, ';');

				  for (k = 0; k < num_semicolon_commands; k++)
                                  {
                    
                                  char *logical_commands[MAX_ARGUMENTS];
                                  int num_logical_commands = split_by_operators(semicolon_commands[k], logical_commands);
